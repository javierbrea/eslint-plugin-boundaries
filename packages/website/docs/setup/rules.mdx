...
Legacy message templates using `${...}` syntax are still supported but deprecated. For migration guidance, see [Legacy Message Templates](#legacy-message-templates)
:::

#### Rule-specific `report` Properties

Some rules provide extra metadata in the `report` object for custom messages. For example, the [`external`](../rules/external.mdx) rule provides details about forbidden specifiers:
