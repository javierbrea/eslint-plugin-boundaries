...
:::warning[Deprecated]
Using `rules` prints a one-time deprecation warning (suppressible via [`boundaries/disable-legacy-warnings`](../settings/settings.md#boundariesdisable-legacy-warnings)) and it will be removed in a future major version. When both `policies` and `rules` are set, `policies` takes precedence.
:::
