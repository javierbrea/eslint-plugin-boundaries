...
:::tip
You can define a custom message at the top level of the rule, which applies to every policy in the `rules` array, or on an individual policy for more specific messages.
:::
...
* `rule` - The policy that produced the error, or `null` when the error comes from the default policy (no policy matched).
...
| `{{rule.selector}}` | object | The full [selector](../selectors/selectors.md#dependency-selectors) (`from`, `to`, `dependency`) of the matching rule — any selector field is available, e.g. `{{rule.selector.from.element.type}}`, `{{rule.selector.to.file.categories}}`, `{{rule.selector.dependency.kind}}` |

Use Handlebars bracket notation to read array items, for example `{{from.element.types.[0]}}` for the first type.

:::tip
For the complete property reference of `from`, `to`, and `dependency`, see [Classification](../classification/classification.md) and its per-layer tables ([element](../classification/elements.md#element-description), [file](../classification/files.md#file-description), [module](../classification/modules.md#module-description)). `rule.selector` is the full selector object of the matching policy (`from`, `to`, `dependency`, each of which can contain `element`, `file`, and `module` sub-selectors) — see [Selectors → Dependency Selectors](../selectors/selectors.md#dependency-selectors) for its complete shape.
:::
...
- [Settings](../settings/settings.md) - configure [`boundaries/files`](../settings/settings.md#boundariesfiles), [`boundaries/elements-single-type`](../settings/settings.md#boundarieselements-single-type), and the other plugin settings.
