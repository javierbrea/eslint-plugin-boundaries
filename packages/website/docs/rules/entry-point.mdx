...
It uses the same default message format as [`boundaries/element-types`](./dependencies.md#error-messages). The message includes information about the violated rule and the elements involved in the violation, as well as details about the specific import that is not allowed (e.g., the internal path of the imported file).
