...
- `rules`: An array of rule objects processed in order to determine whether an import should be allowed. Each rule object contains:
  ...
  - `importKind`: `<string>` - Optional. [TypeScript](../guides/typescript-support.md) only. **Deprecated in v6** (kept for backward compatibility). Use `dependency.kind` instead. If both are defined, `dependency.kind` takes precedence. Possible values: `"value"`, `"type"`, or `"typeof"`
