...
- `rules`: An array of rule objects processed in order to determine whether an import should be allowed. Each rule object contains:
  - `from`: **[`<entity selectors>`](../setup/selectors.md)** - If the file being analyzed matches this selector, the rule is evaluated. Otherwise, it is skipped. Accepts both legacy element selectors (for example `{ type: "helper" }`) and entity selectors (for example `{ element: { type: "helper" } }`)
...
:::warning
Each rule requires `from` and at least one of `allow` or `disallow`.
:::
...
It uses the same default message format as [`boundaries/dependencies`](./dependencies.md#error-messages). Messages include information about the external dependency that is not allowed, such as the module origin and source and, when applicable, the import specifiers and internal path details (read [Classification](../setup/classification.md) for more information on how files and modules are described).
...
* [Selectors](../setup/selectors.md) - Learn how to use element, file, and module selectors (the `to.module` sub-selector matches external dependencies)
