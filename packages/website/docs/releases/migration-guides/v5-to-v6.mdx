...
Version 6.0.0 introduces **[object-based selectors](../../selectors/selectors.md)** as the recommended way to define element selectors. This provides better readability and access to advanced matching features.
...
- **[Disabled `boundaries/no-private` rule in recommended configuration](#recommended-configuration)** - **⚠️ Breaking change**
...
The following **[elements selector](../../selectors/selectors.md)** formats are now deprecated:

1. **String format**: `"helper"`
2. **Tuple format**: `["helper", { family: "data" }]`
3. **Array of legacy selectors**: `["helper", "component"]`

The new **[object-based syntax](../../selectors/selectors.md)** provides:
...
:::info
Read more about the new object-based selector syntax in the **[selectors documentation](../../selectors/selectors.md).**
:::
...
:::info
Read more about the new dependency selector syntax in the [selectors documentation](../../selectors/selectors.md#dependency-selectors).
:::
...
**Rule-level `importKind` is deprecated**: Use **[selector-level `dependency.kind`](../../selectors/selectors.md#dependency-selectors)** instead. When both are defined, selector-level `dependency.kind` takes precedence. This allows for more granular control over matching specific dependency kinds within the same rule.
...
:::note
This does not apply to the [`entry-point` rule](../../rules/entry-point.mdx), because the "allow" and "disallow" properties in that rule are not used to match dependencies but to define the allowed paths to be used as entry points. In that rule, the `importKind` property is still supported. Anyway, that entire rule is also deprecated and will be removed in oncoming versions, so it is recommended to migrate to the `boundaries/dependencies` rule with the new selector syntax as soon as possible. You will find a **[migration example for that rule in its documentation page](../../rules/entry-point.mdx#migration-to-boundariesdependencies)**.
:::

## New template syntax

V6 introduces a new Handlebars-style template syntax (`{{...}}`) for selectors and custom messages, providing clearer syntax and structured access to [element descriptions properties](../../classification/elements.md#element-description). The old `${ }` syntax is still supported for backwards compatibility but will be deprecated in future versions.
...
The new template syntax can access any property from the [dependency description](../../classification/elements.md#element-description), while the old `${to.capturedProperty}` syntax could access only captured properties.

:::tip
Read more about the description properties in the [Runtime Description Properties](../../classification/elements.md#element-description) documentation.
:::
...
:::caution
When the new [`boundaries/legacy-templates` setting](../../settings/settings.md#boundarieslegacy-templates) is enabled (default until next major version), if you are capturing properties with names equal to any of the [Runtime Element Description Properties](../../classification/elements.md#element-description) (e.g., `path`, `category`, `origin`, etc.) they will overwrite the corresponding template variables and cause unexpected behavior in the templates.
...
The new template syntax (`{{ }}`) **can access any property from the [dependency description](../../classification/elements.md#element-description)**, while the **old `${ }` syntax could access only**:
...
:::tip
For more details on the template messages migration, see the Legacy Message Templates section in the [rules documentation](../../policies/legacy.mdx).
:::
...
The boundaries set by this rule now can also be achieved with the **[`boundaries/dependencies` rule](../../rules/dependencies.md)**, which allows you to specify allowed entry points directly in the rules by **using the new [`internalPath` selector property](../../selectors/selectors.md).**
...
The boundaries set by this rule can also be achieved with the **[`boundaries/dependencies` rule](../../rules/dependencies.md)**, which allows you to specify allowed entry points directly in the rules by **using the [`origin`, `source`, `module`, 'internalPath' and `specifiers` selector properties](../../selectors/selectors.md).**
...
The boundaries set by this rule can also be achieved with the **[`boundaries/dependencies` rule](../../rules/dependencies.md)**, which allows you to specify allowed entry points directly in the rules by **using the [`relationship` selector property](../../selectors/selectors.md).**
...
- See [Object-Based Selectors](../../selectors/selectors.md) for complete documentation
- See [Legacy Selectors](../../selectors/legacy/element.md) for reference
