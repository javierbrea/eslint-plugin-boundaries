...
The boundaries set by this rule can also be achieved with the **[`boundaries/element-types` rule](../../rules/dependencies.md)**, which allows you to specify allowed entry points directly in the rules by **using the [`origin` and `source` selector properties](../../setup/selectors.md).**
