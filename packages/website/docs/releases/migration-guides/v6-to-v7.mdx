...
:::tip
The legacy `${ }` template syntax also keeps working in custom messages. Using `${ }` in a message does not print a deprecation warning, and the **[`boundaries/legacy-templates`](../../settings/settings.md#boundarieslegacy-templates)** setting does not affect it — that setting only controls legacy templating in selectors, not in custom messages. For migrating `${ }` templates, see the Legacy Message Templates section in the **[policies legacy documentation](../../policies/legacy.mdx)**.
:::
