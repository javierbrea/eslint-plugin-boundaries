...
- **[Deprecated: `category` in element descriptors and selectors](#deprecated-category-in-elements)** — use file categories
- **[Deprecated: `dependency.module`](#deprecated-dependency-module)** — use `to.module.source`
- **[New custom message template data](#new-template-data)** — `element` / `file` / `module` template objects
...
## Upgrade in 5 minutes {#upgrade-in-5-minutes}
...
- **Legacy `${ }` template warnings.** If your **selector values** (`from` / `to` / `dependency` and `allow` / `disallow`) use the legacy `${ }` syntax, you will keep seeing the v6 deprecation warning. Using `${ }` in a custom `message` does **not** trigger this warning. See [New custom message template data](#new-template-data).
...
## The new entity model {#the-new-entity-model}
...
## Multi-type elements {#multi-type-elements}
...
## Deprecated: `category` in element descriptors and selectors {#deprecated-category-in-elements}
...
## Deprecated: `dependency.module` {#deprecated-dependency-module}
...
## New custom message template data {#new-template-data}
...
## `no-unknown-files` behavior change {#no-unknown-files-behavior-change}
...
## TypeScript-only breaking changes {#typescript-only-breaking-changes}
