...
- **[Deprecated: `mode` in element descriptors](#deprecated-mode-in-element-descriptors)** — use `partialMatch: false` instead of `mode: "full"`
...
- **[`no-unknown` renamed to `no-unknown-dependencies`](#no-unknown-renamed-to-no-unknown-dependencies)** — file-aware, with new options
- **[`no-ignored` renamed to `no-ignored-dependencies`](#no-ignored-renamed-to-no-ignored-dependencies)** — rule name change only
