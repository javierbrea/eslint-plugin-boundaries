...
import {
  NO_IGNORED_DEPENDENCIES as RULE,
  RULE_NAMES_MAP,
} from "../../../src/Shared";
import {
  SETTINGS,
...
const rule = getRule();
const deprecatedRule = getRule(RULE_NAMES_MAP.NO_IGNORED);
...
// Deprecated rule name `boundaries/no-ignored` keeps working (with the same
// behavior); it emits a one-time rename warning at lint time.
createRuleTester({
  ...settings,
  "boundaries/ignore": ["**/foo.js"],
} as RuleTesterSettings).run(RULE, deprecatedRule, {
  valid: [
    {
      filename: absoluteFilePath("helpers/data/sort.js"),
      code: "import foo2 from '../../foo2'",
