...
- feat: Add `boundaries/disable-legacy-warnings` boolean setting (default `false`). When `true`, the plugin skips all legacy-pattern detection work — element descriptor deprecated-field checks and rule option deprecated-syntax scans — and suppresses the related deprecation warnings, reducing overhead at lint time.
