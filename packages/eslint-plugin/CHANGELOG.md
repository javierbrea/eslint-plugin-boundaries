...
- feat: Emit a one-time runtime console deprecation warning when a rule uses a legacy element selector — a legacy string or tuple selector, or a plain element-selector object (e.g. `{ type: "component" }`) used directly in `from`/`to`/`target`/`allow`/`disallow` instead of wrapped in an entity selector (`{ element: { type: "component" } }`) — encouraging migration to object-based entity selectors. The warning is harmless and does not change results.
