...
- feat: The `strict` config now enables `boundaries/no-ignored-dependencies` and `boundaries/no-unknown-dependencies` instead of `boundaries/no-ignored` and `boundaries/no-unknown`. If your codebase has `eslint-disable` comments referencing the old rule names, they stop suppressing the corresponding violations (now reported under the new rule names) and may themselves trigger "unused eslint-disable directive" errors. Update the comments to the new rule names (recommended, and required for a future major version), or use the new `strict-legacy` config in the meantime, which keeps enabling the old rule names.
